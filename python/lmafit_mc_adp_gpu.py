...
def rank_estimator_adaptive(k,iter,est_rank,reschg,tol,rk,rank_max,itr_rank,R,rk_jump, minitr_reduce_rank, maxitr_reduce_rank,rank_min,Zfull,iprint,alf,rk_inc,save_res,X,Y,m,n):
    est_rank=torch.tensor(est_rank).cuda() # 将 est_rank 转换为 PyTorch tensor 并将其移至 GPU
    if est_rank == 1:# 如果 est_rank 等于 1
        ###no###
        
        dR = abs(np.diag(R))# 计算矩阵 R 的对角线的绝对值
        drops = dR[0:-2]/dR[1:-1]# 计算相邻奇异值之间的比率
        dmx,imx = max(drops)# 获取最大的奇异值比率及其索引
        rel_drp = (k-1)*dmx/(sum(drops)-dmx) # 计算相对奇异值下降率


        #???
        if  ((rel_drp > rk_jump) & (itr_rank > minitr_reduce_rank)) | (itr_rank > maxitr_reduce_rank):
            
            rk = max([imx, math.floor(0.1*k), rank_min])# 更新矩阵秩 rk
            X = X[:,1:rk]; Y = Y[1:rk,:]# 更新矩阵 X 和 Y
            if Zfull:# 如果 Zfull 为真
               

                Z =np.dot(X,Y) # 计算矩阵 Z
                xx=Z.shape[0]# 获取 Z 的行数
                yy=Z.shape[1]# 获取 Z 的列数
                Z=np.reshape(Z.T,[Z.shape[0]*Z.shape[1],1])# 对 Z 进行重塑和转置操作
                Res = data - Z[Known]  # 计算残差 Res
                Z[Known] = data + alf * Res  # 更新 Z
                Z=np.reshape(Z,[yy,xx]) # 对 Z 进行重塑
                Z=Z.T# 对 Z 进行转置操作
            
            else:#不执行未修改
                
                Res = data - partXY(X.T,Y,Ik,Jk,L)# 计算残差 Res
                updateSval(S, (alf+1)*Res, L)# 更新 S 的值
            res = np.linalg.norm(Res)# 计算残差的范数
            est_rank = 0 # 将 est_rank 设置为 0
            itr_rank = 0# 将 itr_rank 设置为 0
            if iprint >= 2: # 如果 iprint 大于等于 2
                ##format  未完全设置格式精度
        ...
        if rk < 50: 
            ...
            rinc = 2*rk_inc
        rk = min(rk + rinc, rank_max)# 更新秩 rk
        rkr_id = True
        if rk > k:###yes###
            if save_res == 1:  ###no###
